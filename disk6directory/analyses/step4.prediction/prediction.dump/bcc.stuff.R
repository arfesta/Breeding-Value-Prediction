...
unlist(lapply(family_anova_scores,function(x) length(which(x < .05))))
# How many of anova scores are less than .05 (adjusted)
unlist(lapply(family_anova_scores,function(x) length(which(p.adjust(x) < .05))))
```
...
EW.test.dat <- fam.012[which(fam.phenos$fam_id %in% ew.fams),which(p.adjust(anova_scores) < .05)]
colnames(EW.test.dat) <- colnames(fam.012[,which(p.adjust(anova_scores) < .05)])
...
```{r}
interesting.bcv.snps2 <- which(var.snps[which(bcv.snps > 3)] > .2)
interesting.bcv.snps2 <- which(bcv.snps > 3)
interesting.bcv.snps2 <- near.zero.bcv3.snps
each.fam=1
train.dat <- fam.012[-each.fam,interesting.bcv.snps2]
colnames(train.dat) <- colnames(fam.012)[interesting.bcv.snps2]
train.dat.sd <- apply(train.dat,2,sd)
train.dat.mean <- apply(train.dat,2,mean)
train.dat.bcv <- train.dat.sd/train.dat.mean
hist(train.dat.bcv)
#train.dat <- train.dat[,which(train.dat.bcv > 3)]
#test.dat <- data.frame(fam.012[each.fam,interesting.bcv.snps])
