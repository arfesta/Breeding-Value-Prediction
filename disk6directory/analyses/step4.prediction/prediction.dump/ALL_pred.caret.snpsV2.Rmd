...
```{r fam_anova}
load("/mnt/media/disk6/ARF/shared/LOO_fam_snp_anova.RData")
# How many of anova scores are less than .05 (unadjusted)
hist(unlist(lapply(bio_loo_anova_scores,function(x) length(which(x < .05)))))
# How many of anova scores are less than .05 (adjusted)
hist(unlist(lapply(bio_loo_anova_scores,function(x) length(which(p.adjust(x) < .05)))))
```
...
```{r }
tC <- trainControl(method="LGOCV",allowParallel = F, savePredictions="all",verboseIter = F,search="grid")
...
for(each.mod in 1:length(algorithmList)){
  alpha <- .2
 fam.preds <- unlist(mclapply(1:56,function(each.fam){
    train.dat <- fam.012.df[-each.fam,which( p.adjust(bio_loo_anova_scores[[each.fam]]) < alpha)]
    train.p <- fam.phenos$Volume[-each.fam]
 
    test.dat <- fam.012.df[,which( p.adjust(bio_loo_anova_scores[[each.fam]]) < alpha)]
 	    set.seed(123)
 	    t2 <- train(y=train.p, x=train.dat,"glmnet", trControl = tC)
      unlist(predict(t2,test.dat)[each.fam])
    },mc.cores=30))
 fam.pred.mat[,each.mod] <- fam.preds
 }

t2 <- lapply(algorithmList,function(i){ 
  this.alg <- algorithmList[[i]]
	set.seed(123)
  mclapply(1:56,function(each.fam){
	 t2 <- train(y=LGEP.train.p, x=LGEP.train.dat, this.alg, trControl = tC)
	 predict(t2, EW.test.dat)
	 },mc.cores=30)
})
