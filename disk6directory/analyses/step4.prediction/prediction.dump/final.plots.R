...
initial.loadsnps <- function(){
  bio.hm.012.df <- read.table("./snps/012/192.biorep.Q30.snps.no.miss.maf05.ann.high.mod.012",header = F,row.names = 1)
  rownames(bio.hm.012.df) <- as.character(read.table("./snps/012/192.biorep.Q30.snps.no.miss.maf05.ann.high.mod.012.indv")[,1])
  colnames(bio.hm.012.df) <- paste0(as.character(read.table("./snps/012/192.biorep.Q30.snps.no.miss.maf05.ann.high.mod.012.pos")[,1]),
                                 "_",
                                 read.table("./snps/012/192.biorep.Q30.snps.no.miss.maf05.ann.high.mod.012.pos")[,2])
  
  # Remove any extra "_" characters in the sample names
  rel.colnames <- gsub(rownames(bio.hm.012.df),pattern = "_",replacement = "")
  ...
  head(rel.colnames)
  rownames(bio.hm.012.df) <- rel.colnames
  rm(rel.colnames,rel.colnames.ew,ew.bio.reps,lgep.bio.reps,mc.ew,mc.lgep)
  save(bio.hm.012.df,file="/mnt/bio.hm.012.df.RData",compress=T)
}
...
load.bio.data <- function(){
  ...
  keep.index <-  which(rownames(bio.hm.012.df) %in% bio.phenos.snps$animal_id)
  
  # Keep only those rows
  bio.hm.012.df <- bio.hm.012.df[keep.index,]
  
  bio.phenos.snps <- bio.phenos.snps[which(bio.phenos.snps$animal_id %in% rownames(bio.hm.012.df)),]
  identical(bio.phenos.snps$animal_id[match(rownames(bio.hm.012.df),bio.phenos.snps$animal_id)],rownames(bio.hm.012.df))
  
  bio.phenos.snps <- bio.phenos.snps[match(rownames(bio.hm.012.df),bio.phenos.snps$animal_id),]
  ...
  rownames(asreml.counts) <- rownames(bio.hm.012.df)
...
create.family.data <- function(bio.data.list=NULL){
  ...
  bio.hm.012.df <- as.matrix(bio.hm.012.df)
  fam.hm.012.df <- matrix(NA,nrow=56,ncol=ncol(bio.hm.012.df))
  for(each.fam in 1:56){
  merge.bio.reps <-  which(bio.phenos$fam_id %in% u.fams[each.fam])
    if(length(merge.bio.reps) == 1){ fam.hm.012.df[each.fam,] <- bio.hm.012.df[merge.bio.reps,] } else {
      fam.hm.012.df[each.fam,] <-apply(bio.hm.012.df[merge.bio.reps,],2,mean)}
  }
  rownames(fam.hm.012.df) <- u.fams
  colnames(fam.hm.012.df) <- colnames(bio.hm.012.df)
  save(fam.hm.012.df,file="/mnt/fam.hm.012.df.RData",compress=T)
...
t.test(as.numeric(gp1.cor[,3]),as.numeric(gp2.cor[,3]))
