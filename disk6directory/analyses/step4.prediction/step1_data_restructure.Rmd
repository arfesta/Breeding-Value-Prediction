...
## Consolidate phenos to bioliogical reps
...
bio.phenos <- unique(expt.dat.720[,c(1,2,13,17)])
...
fam.counts <- mclapply(1:56,function(each.fam){
  these.rows <- which(as.character(bio.phenos$fam_id) %in% rownames(fam.phenos))
  apply(asreml.counts[these.rows,],2,mean)
},mc.cores=8)
