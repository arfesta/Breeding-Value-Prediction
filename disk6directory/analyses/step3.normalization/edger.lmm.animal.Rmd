...
```{r}
pt <- proc.time()
    asreml.edger.counts <- mclapply(1:nrow(pre.norm.counts),function(each.txpt){
...
```{r}
asreml.edger.counts <- do.call(cbind,asreml.edger.counts)
asreml.edger.counts <- asreml.edger.counts[-c(1:94),]
mypc <- prcomp(2^asreml.edger.counts,scale. = T,center = T)
mypc <- mypc$x
plot(mypc[,1:2])

colnames(asreml.edger.counts) <- rownames(pre.norm.counts)
asreml.edger.counts <- 2^asreml.edger.counts
asreml.edger.counts.animal <- asreml.edger.counts
save(asreml.edger.counts.animal,file = "../Step3_edgeR_LMM.animal_Counts.RData",compress=T)    
```  
