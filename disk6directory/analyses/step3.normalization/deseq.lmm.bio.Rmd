...
```{r}
pt <- proc.time()
    asreml.de.counts.bio <- mclapply(1:nrow(pre.norm.counts),function(each.txpt){
      #each.txpt=323
    ...
    tmp1 <- asreml(fixed= txpt ~1 + batch  + true.index, random = ~ ide(fam_id,var=T,init=1) + ped(fam_id,var=T,init=1),
...
```{r}
asreml.de.counts.bio <- do.call(cbind,asreml.de.counts.bio)
rm.zero.var <- (which(apply(asreml.de.counts.bio,2,function(x) {var(x)}) == 0))
asreml.de.counts.bio <- asreml.de.counts.bio[,-rm.zero.var]
mypc <- prcomp(2^asreml.de.counts.bio,scale. = T,center = T)
mypc <- mypc$x
plot(mypc[,1:2])

edit.names <- gsub(pattern = "fam_id, var = T)_",replacement = "",x = rownames(asreml.de.counts.bio))
      edit.names <- gsub("ped\\(", "", edit.names)
    
rownames(asreml.de.counts.bio) <- edit.names
colnames(asreml.de.counts.bio) <- rownames(pre.norm.counts)[-rm.zero.var]
asreml.de.counts.bio <- 2^asreml.de.counts.bio
save(asreml.de.counts.bio,file = "../Step3_DE_LMM_bio_Counts.RData",compress=T)    
```  
