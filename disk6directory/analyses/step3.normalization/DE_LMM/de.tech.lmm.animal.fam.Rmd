...
pt <- proc.time()
    ...
    tmp1 <- asreml(fixed= txpt ~1 + batch + lane + index_seq, random = ~ ide(animal_id,var=T,init=1) + ped(fam_id,var=T,init=1),
               ginverse=list(fam_id=ainv,animal_id=ainv2),data=tip.subset,maxiter=50,trace=F,
...
edit.names <- gsub(pattern = "fam_id, var = T)_",replacement = "",x = rownames(asreml.de.counts))
      edit.names <- gsub("ped\\(", "", edit.names)
    
rownames(asreml.de.counts) <- edit.names
colnames(asreml.de.counts) <- rownames(pre.norm.counts)
asreml.de.counts <- 2^asreml.de.counts
save(asreml.de.counts,file = "../Step3_DE_LMM_Counts.RData",compress=T)    
```  
