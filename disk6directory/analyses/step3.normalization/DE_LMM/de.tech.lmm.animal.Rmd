...
ddsTxi <- DESeqDataSetFromTximport(load.counts_tech$txi_object,
                                   colData = load.counts_tech$phenos,
...
```{r}
pt <- proc.time()
    asreml.de.counts <- mclapply(1:nrow(pre.norm.counts),function(each.txpt){
      #each.txpt=323
    ...
    #edit.names <- gsub(pattern = "animal_id, var = T)_",replacement = "",x = names(tmp1$coefficients$random))
    #  edit.names <- gsub("ped\\(", "", edit.names)
    #my.fams <- which(edit.names %in% unique(as.character(load.counts_tech$phenos$fam_id)))
    my.counts <- tmp1$coefficients$random + tmp1$coefficients$fixed['(Intercept)']
...
```{r}
asreml.de.counts <- do.call(cbind,asreml.de.counts)
asreml.de.counts <- asreml.de.counts[-c(1:94),]
mypc <- prcomp(2^asreml.de.counts,scale. = T,center = T)
mypc <- mypc$x
plot(mypc[,1:2])

colnames(asreml.de.counts) <- rownames(pre.norm.counts)
asreml.de.counts <- 2^asreml.de.counts
asreml.de.counts.animal <- asreml.de.counts
save(asreml.de.counts.animal,file = "../Step3_DE_LMM.animal_Counts.RData",compress=T)    
```  
