...
```{r}
pt <- proc.time()
    asreml.edge.counts.bio <- mclapply(1:nrow(pre.norm.counts),function(each.txpt){
...
```{r}
asreml.edge.counts.bio <- do.call(cbind,asreml.edge.counts.bio)
mypc <- prcomp(2^asreml.edge.counts.bio,scale. = T,center = T)
mypc <- mypc$x
plot(mypc[,1:2])

edit.names <- gsub(pattern = "fam_id, var = T)_",replacement = "",x = rownames(asreml.edge.counts.bio))
      edit.names <- gsub("ped\\(", "", edit.names)
    
rownames(asreml.edge.counts.bio) <- edit.names
colnames(asreml.edge.counts.bio) <- rownames(pre.norm.counts)
asreml.edge.counts.bio <- 2^asreml.edge.counts.bio
save(asreml.edge.counts.bio,file = "../Step3_edgeR_LMM_bio_Counts.RData",compress=T)    
```  
