...
```{r}
pt <- proc.time()
    asreml.edger.fam <- mclapply(1:nrow(pre.norm.counts),function(each.txpt){
...
```{r}
asreml.edger.fam <- do.call(cbind,asreml.edger.fam)
mypc <- prcomp(2^asreml.edger.fam,scale. = T,center = T)
mypc <- mypc$x
plot(mypc[,1:2])

colnames(asreml.edger.fam) <- rownames(pre.norm.counts)
asreml.edger.fam <- 2^asreml.edger.fam
save(asreml.edger.fam,file = "../Step3_edgeR_LMM.fam_Counts.RData",compress=T)    
```  