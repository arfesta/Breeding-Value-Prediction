...
load("./step1/salmon_count_data.v2.RData")
load("./step1/salmon_expt_data.v2.RData") 
...
asreml.ped <- data.frame(full.pedigree[,c("animal.id","p1.animal.id","p2.animal.id")])
...
pre.norm.counts <- data.frame(apply(salmon.raw.counts,2,as.integer))
raw.sum <- apply(pre.norm.counts,2,sum)
raw.mean <- apply(pre.norm.counts,2,mean)
#pre.norm.data <- data.frame(apply(salmon.raw.counts,2,as.integer))
#raw.sum <- apply(pre.norm.counts,2,sum)
#rm.txpt <- which(raw.sum <= 1)
#rm.txpt <- which(raw.sum <= 576)
#rm.txpt <- which(raw.mean <= 30)
pre.norm.data <- pre.norm.counts[,-rm.txpt]
pre.norm.length <- salmon.raw.lengths[,-rm.txpt]
expt.dat.tmp <- data.frame(expt.dat.576[,c("animal.id","true.index","lane.id")],stringsAsFactors = T)

#asreml.counts <-  mclapply(1:ncol(pre.norm.data),function(each.txpt){
each.txpt=1
  tip.subset <- data.frame("y"=pre.norm.data[,each.txpt],expt.dat.tmp,"length" = pre.norm.length[,each.txpt])
  tip.subset <- within(tip.subset,{y <- (as.integer(y)); 
          animal.id <- factor(animal.id); 
          true.index <- factor(true.index); 
          lane.id <- factor(lane.id);
          length <- as.numeric(length)})
  tmp1 <- asreml(fixed= y ~ lane.id + true.index , random = ~ ped(animal.id),
    ginverse=list(animal.id=ainv),data=tip.subset,maxiter=500,trace=F, offset=length,
    family=asreml.poisson(link="identity"))
  plot(tmp1$coefficients$random[79:270])
#},mc.cores=22)
asreml.counts <- do.call(cbind,asreml.counts)
rownames(asreml.counts) <- names( tmp1$coefficients$random[79:270] )
colnames(asreml.counts) <- colnames(pre.norm.data)
save(asreml.counts,file = "./step2.normalization/asreml_normalization/asreml.counts.RData",compress=T)
save(expt.dat.192,file = "./step2.normalization/asreml_normalization/192.expt.data.Rdata",compress=T)
save.image(file = "./step2.normalization/asreml_normalization/asreml.all.RData",compress=T)
