...
pre.norm.lengths <- salmon.raw.lengths[,-rm.txpt]
...
asreml.counts <-  mclapply(1:ncol(pre.norm.data),function(each.txpt){
  tip.subset <- data.frame("y"=pre.norm.data[,each.txpt],expt.dat.tmp,"length"= pre.norm.lengths[,each.txpt])
  tip.subset <- within(tip.subset,{y <- (as.integer(y)); 
  animal.id <- factor(animal.id); true.index <- factor(true.index); 
  lane.id <- factor(lane.id); length <- as.numeric(length)})
  tmp1 <- asreml(fixed= y ~ lane.id + true.index , random = ~ ped(animal.id),ginverse=list(animal.id=ainv),
    data=tip.subset,maxiter=30,trace=F, offset = length,
    family = asreml.poisson(link="log"),debug = F)
  
  
  hist(tmp1$coefficients$random[79:270])
},mc.cores=22)
...
save(asreml.counts,file = "./step2.normalization/asreml_normalization/asreml.counts.offset.RData",compress=T)
save(expt.dat.192,file = "./step2.normalization/asreml_normalization/192.expt.data.offset.Rdata",compress=T)
save.image(file = "./step2.normalization/asreml_normalization/asreml.all.offset.RData",compress=T)
