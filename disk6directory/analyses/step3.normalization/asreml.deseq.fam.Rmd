...
```{r}
pt <- proc.time()
    asreml.de.counts.fam <- mclapply(1:nrow(pre.norm.counts),function(each.txpt){
    ...
    tmp1 <- asreml(fixed= txpt ~1 + batch + lane + index_seq, random = ~ ped(fam_id,var=T,init=1),
...
```{r}
asreml.de.counts.fam <- do.call(cbind,asreml.de.counts.fam)
mypc <- prcomp(2^asreml.de.counts.fam,scale. = T,center = T)
mypc <- mypc$x
plot(mypc[,1:2])

edit.names <- gsub(pattern = "fam_id, var = T)_",replacement = "",x = rownames(asreml.de.counts.fam))
      edit.names <- gsub("ped\\(", "", edit.names)
    
rownames(asreml.de.counts.fam) <- edit.names
colnames(asreml.de.counts.fam) <- rownames(pre.norm.counts)
asreml.de.counts.fam <- 2^asreml.de.counts.fam
save(asreml.de.counts.fam,file = "../Step3_DE_LMM_Fam_Counts.RData",compress=T)    
```  
