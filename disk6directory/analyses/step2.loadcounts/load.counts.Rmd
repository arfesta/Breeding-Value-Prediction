...
This script takes the raw bioloigcal replicate count data produced by the first script (analyses/step1.data_prep/README.md) and preps objects for normalization techniques. Given that the objective of this project is to predict phenotypes, we will subset the counts and phenotype data to only include families where we have phenotypes available to use.
...
* Load complete experimental meta data
    ...
    * Note that the experimental data needs to be consolidated to bioloigcal replicates in order to match the txi object.  This will be done next by identifying the column names and removing certain ones which uniquely describe the techincal replicates.

## Convert objects to bioloigcal replicates
* Print the column names of the experimental data frame.
    ```{r meta.colnames, cache=F, eval=TRUE}
    colnames(expt.dat.720)
    ```

      * Lot's of info in here.  The ones which are unique to technical replicates within a given biological replicate are:
          * lane (col 6)
          * fl_mean (col 8)
          * salmple_id (col 10)
...
* Remove the above columns and create bioloigcal replicate experimental data set
...
## Subset families which have phenotypes

Since this script will only utilize families which have phenotypes, let's identify those families and subset them from the data frames

* Identify families which are missing phenotypic data for volume
    ```{r subset.ew.lgep, cache=F }
    missing.phenos <- which(is.na(expt.dat.240$Volume) == T)
    table(as.character(expt.dat.240$fam_id)[missing.phenos])
    ```
      
    * Note that family N051049 has a phenotype in tiproot version Piedmont 2014 V1, but does not have phenotype vaule for 2017 V1 Coastal. Should this be included?
    
* Subset these families from the phenotype matrix
    ```{r subset.ew.lgep.pheno, cache=F }
    sub.phenos <- droplevels.data.frame(expt.dat.240[-c(missing.phenos),])
    ```

* Subset these families from txi object
    ```{r rm.low.cts, cache=F }
    # Remove families with missing phenos from counts
    txi$counts <- txi$counts[,-c(missing.phenos)]
    ...
    # Remove low counts and remove fams with missing pheno from txi object
    txi$abundance <- txi$abundance[-c(rm.zero),-c(missing.phenos)]
    txi$length <- txi$length[-c(rm.zero),-c(missing.phenos)]
    txi$variance <- txi$variance[-c(rm.zero),-c(missing.phenos)]
    ```

## Pre-filter data to remove outliers
...
* Remove outlier biological replicates
    ...
    sub.phenos <- droplevels.data.frame(sub.phenos[-c(rm.bio.reps),])
