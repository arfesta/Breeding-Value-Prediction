#compare things with volume####
load("~/Desktop/prediction.paper/top.roundrobin.snp.vol.RDA")
...
load("~/Desktop/prediction.paper/3.2.2/top.de.fdr.01.vol.RDA")
library(OmicKriging)

#Compare weighing SNPs found from RR LOO analysis vs. top DE transcripts found####
cor.txt <- cor(t(scaled.fam.logcpm2[,txts.01])); colnames(cor.txt) <- 1:56; rownames(cor.txt) <- 1:56
se <- seq(.01,.99,.01)
cor.list <- vector()
...
for(i in 1:56){
  ...
  tmp <- okriging(idtest=test,idtrain=seq(1,56,1)[-c(test)],corlist = list(cor.txt,cor.snp),
                  H2vec = c(se[[each]],1-se[[each]]),pheno = phenos.fam,phenoname = "vol")
  pred <- c(pred,tmp[,2])
  true <- c(true,tmp[,3])
 cor(tmp[,2],tmp[,3])}
plot(pred,true)
cor.list <- c(cor.list,cor(pred,true)^2)}
...
for(each in 1:length(se)){
  ...
  for(i in 1:56){
    ...
    test <-  i
    tmp <- okriging(idtest=test,idtrain=seq(1,56,1)[-c(test)],corlist = list(cor.txt,cor.snp),
                    H2vec = c(se[[each]],.99-se[[each]]),pheno = phenos.fam,phenoname = "vol")
    pred <- c(pred,tmp[,2])
    true <- c(true,tmp[,3])
    cor(tmp[,2],tmp[,3])}
  plot(pred,true)
  cor.list2 <- c(cor.list2,cor(pred,true)^2)}
