...
lm.bio.rep.snp.22k.ht <- mclapply(1:56,FUN=function(x){
...
for(each in 1:length(se)){
  ...
  for(i in 1:56){
    c <- table(names(all.55.22.qvalues[[i]])[which(all.55.22.qvalues[[i]] <= .05)])                       
    ...
    tmp <- okriging(idtest=test,idtrain=seq(1,56,1)[-c(test)],corlist = list(cor.matrix),
                    H2vec = c(.8),pheno = phenos.fam,phenoname = "ht")
    pred <- c(pred,tmp[,2])
    true <- c(true,tmp[,3])
...
pdf(file = "~/Desktop/snp.lmoo.ht.pdf",width = 4,height = 4)
...
top.roundrobin.snp.ht <- vector("list")
for(i in 1:56){
  c <- table(names(all.55.22.qvalues[[i]])[which(all.55.22.qvalues[[i]] <= .05)])                       
  vars <- names(c)[which(c >= 55)]
  top.roundrobin.snp.ht[[i]] <- vars}
save(top.roundrobin.snp.ht,file="~/Desktop/prediction.paper/top.roundrobin.snp.ht.RDA")
